...
int main(void)
{
  ...
  vector <int> rocky;
  vector <double> borisd(5);
  vector <char> bw(5,'Q');
  vector <string> natasha (3,"hello");
  vector <int> cloyd(peachfuzz,peachfuzz+3);
  int i;
  cout<<"rocky cap: " <<rocky.capacity()<< "rocky size: " <<rocky.size()<<endl;
 // cout<<"boris cap: " <<boris.capacity()<< "boris size: " <<boris.size()<<endl;
  cout<<"bw cap: " <<bw.capacity()<< "bw size: " <<bw.size()<<endl;
